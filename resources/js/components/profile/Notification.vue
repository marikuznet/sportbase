<template>
    <div>
                    ...
                    <ul class="navbar-nav nav navbar-collapse d-lg-none profile__user">
                        <li v-on:click.prevent="showOrHideMenu"
                            ...
                            <a class="nav-link text-white">
                                <img class="img__icon-settings" src="/icons/settings/notificationWhite.svg" alt="">
                                Оповещания</a>
                        ...
                        <li v-bind:class="{ 'border__grey': showProfile }" class="nav-item mx-auto">
                            <a href="/profile/settings"
                               class="nav-link text-white userIcon">
                                <img class="img__icon-settings" style="width: 20px" src="/icons/settings/userWhite.svg"
                                     alt="">
                                Профиль пользователя</a>
                        <li v-bind:class="{ 'border__grey': showProfile }" class="nav-item mx-auto">
                            <a class="nav-link text-white" href="/profile/payment">
                                <img class="img__icon-settings" src="/icons/settings/real-estateWhite.svg" alt="">
        ...
        <main class="main-notification" style="min-height: 600px">
            <div class="container">
                <div class="row">
                    <div class="col-lg-4">
                        <left-menu :active="2"></left-menu>
                    </div>
                    <div class="col-lg-8">
                        <div class="block-notification shadow h-100">
                            <div class="title__block d-none d-lg-block">
                                <h3 class="caption p-4">Оповещения</h3>
                            </div>

                            <div class="p-lg-4 pb-4 pt-4 sendNotification">
                                <h5 class="caption">Отправка уведомлений на email адрес</h5>
                                <div v-if="error" class="form-wrapper__error mt-1 mb-3">
                                    ...
                                    <span class="pl-3 pt-1 text-danger">{{ error }}</span>
                                </div>
                                <label class="w-100 subtitle pt-1" for="">Email адрес
                                    <input type="text" name="search-object" :class="{ error: emailError }"
                                           v-model="email" autocomplete="off"
                                           placeholder="mail@mail.com" class="input-profile-info mt-2">
                                </label>

                            </div>

                            <div class="block_events p-lg-4">
                                <h5 class="caption">События</h5>
                                <div class="row pt-2">
                                    <div class="col-lg-6" v-for="(item,index) in allNotifications" :key="item.id">
                                        <label class="d-flex float-left">
                                            <input class="checkEvent d-none" type="checkbox" :value="item.id"
                                                   v-model="notifications">
                                            <span v-bind:class="{ checked: isActiveNotif }"></span>
                            ...
                            <div class="pb-4">
                                <form @click="setNotificationEmail">
                                    <button type="button"
                                            class="button button-green button-center d-block">Сохранить
                                    </button>
                                </form>
                            </div>
                        </div>
...
export default {
    data: function () {
        return {
            showProfile: false, eventNotification: [], checked: [], email: '', emailError: '',
            notifications: [], isActiveNotif: false, userInfo: {}
        }
    },
    computed: {
        ...mapGetters({
        ...
        getNotifications() {
            if (this.userInfo && this.userInfo.notifications && this.userInfo.notifications.length > 0) {
    ...
    methods: {
        ...mapActions(["fetchEventNotifications"]),
        showOrHideMenu() {
        ...
        async setNotificationEmail() {
            this.emailError = "";
            if (!this.email) {
                return (this.emailError = "Введите почту");
            }

            await axios.get('/profile/sendNotification/destroy');
            for (let i = 0; i < this.notifications.length; i++) {
                axios.post('/profile/sendNotification/create', {
                    'email': this.email,
                    'eventNotification_id': this.notifications[i],
                    'user_id': user.id
                }).then(response => {
                }).catch((error) => {
        ...
        getNotifications: {
            handler: function (info) {
                ...
                for (let i = 0; i < info?.length; i++)
                    this.notifications.push(info[i]?.event_id);
            }, deep: true
        }
    },
    mounted() {
